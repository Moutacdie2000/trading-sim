...
import { useEngineFeed, type FeedState } from '../useEngineFeed.js';
...
function makeFakeSocket(url: string): FakeWebSocket {
...
const FakeWebSocketCtor = function(this: FakeWebSocket, url: string) {
  const s = makeFakeSocket(url);
  Object.assign(this, s);
  sockets[sockets.length - 1] = this as unknown as FakeWebSocket;
} as unknown as typeof WebSocket;

let container: HTMLDivElement;
let root: Root;
let captured: FeedState | null = null;
...
describe('useEngineFeed', () => {
  it('reconnects with exponential backoff after a forced close', () => {
    act(() => { root.render(<Probe url="ws://x/feed" />); });
