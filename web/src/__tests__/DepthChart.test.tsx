...
describe('computeDepth', () => {
  ...
  it('produces deterministic SVG path data for a known input', () => {
    const bids: [number, number][] = [[100, 5], [99, 3]];
    const asks: [number, number][] = [[101, 4], [102, 2]];
    ...
    expect(computed!.bidPath).toMatchInlineSnapshot(
      `"M 200.00 222.00 L 200.00 222.00 L 136.00 222.00 L 136.00 88.25 L 8.00 88.25 L 8.00 8.00 L 0.00 8.00 L 0.00 222.00 Z"`,
    );
    expect(computed!.askPath).toMatchInlineSnapshot(
      `"M 200.00 222.00 L 200.00 222.00 L 264.00 222.00 L 264.00 115.00 L 392.00 115.00 L 392.00 61.50 L 392.00 61.50 L 392.00 222.00 Z"`,
    );
  });

  it('cumulates quantities across multiple levels', () => {
    const bids: [number, number][] = [[100, 1], [99, 2], [98, 3]];
    const asks: [number, number][] = [[101, 1], [102, 2]];
    const computed = computeDepth(bids, asks, 400, 240);
